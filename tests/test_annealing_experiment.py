...
import combinators.v2.inference as V2
...
def experiment_runner(trainer):
    ...
    with trange(num_iterations) as bar:
        for i in bar:
            lw, lvss, loss = trainer(targets, forwards, reverses, sample_shape)
            loss.backward()
...
def nvi_eager(targets, forwards, reverses, sample_shape):
    q0 = targets[0]
    ...
    for k, (fwd, rev, q, p) in enumerate(zip(forwards, reverses, targets[:-1], targets[1:])):
        ...
        q_ext = V2.Forward(fwd, q, _step=k)
        p_ext = V2.Reverse(p, rev, _step=k)
        extend = V2.Propose(target=p_ext, proposal=q_ext, _step=k)
...
def test_eager_annealing():
    experiment_runner(nvi_eager)
