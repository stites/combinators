...
@typechecked
def log_weights(target_trace:Trace, proposal_trace:Trace, sample_dims:int=None, validate:bool=True):
    if validate:
...
class Forward(KernelInf, Inf):
    ...
    def forward(self, *program_args:Any, sample_dims=None, **program_kwargs) -> Tuple[Trace, Output]:
        ...
        # log_weights(kernel_state.trace, proposal_trace, sample_dims=None, validate=True):
...
class Propose(nn.Module, Inf):
    ...
    def forward(self, *shared_args, sample_dims=None, **shared_kwargs):
        ...
        lv = log_weights(joint_target_trace, joint_proposal_trace, validate=self.validate, sample_dims=sample_dims)
        # print(self._cache)
